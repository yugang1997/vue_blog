    ...
    <el-container>
      ...
      <el-aside>
        <div :class="['search',{'fixed': ifSearchFixed}]">
          ...
          <div class="s-articleList">
            <ul @mouseleave="handleMouseLeave">
              <li @mouseenter="handleMouseEnter(index)" v-for="(item,index) in getArticleTags" :key="item.id">
        ...
        <div class="ArticleHot">
          <h3>热门文章</h3>
          <ul>
            <li v-for="(item,index) in articleHot" :key="item.id">
              <i>{{index+1}}</i>
              <router-link :to="'/Article/'+item._id">{{item.title}}</router-link>
            </li>
          </ul>
        ...
        <div class="visitor">
          <h3>最近访客</h3>
          <ul>
            <li v-for="item in visitor" :key="item.id" :style="{backgroundImage: 'url('+item.user.photo+')'}">
              <p>{{item.user.user}}</p>
            </li>
          </ul>
        </div>
